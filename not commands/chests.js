...
module.exports = (client) => {
  client.on("message", async (message) => {
    if (
      message.channel.id == "853386908376629309" ||
      message.channel.id == "838588912041066496"
    )
      return;
    setTimeout(async () => {
            ...
            if (randomAmount == 12) {
              ...
              if (!userResult || !userResult.money)
                return message.reply(
                  "You would have just gotten a chest, but since you've never used our economy it was voided (canceled)"
                );
              if (userResult.econverified == "false") return;
              let chestAmount = 0;
